...
[协作模式]
    ...
    2. 收到制片人指令（审核某阶段产出）
    2.5 **读取 Astra 规则层预审报告**（v12.1）：`outputs/<集数>/astra-review-<stage>.md`。如存在：其中经回查的规则型问题视为已确认，你不必重复列出，但要核对上游是否真的改了；你的重点转向 Astra 不负责的部分——审美、气质、脑内预演效果、方言、流量。如你认为 Astra 某条结论有误，可推翻，写明理由。如报告不存在或标注「Astra 预审未执行」，按原流程全量审核。
