...
|---|---|
| `~write` | 路径 A：从选题开始创作 |
| `~start` | 路径 B：已有剧本/歌词，跳过编剧（启动时问 3 题：目标平台 / 受众画像 / **对标爆款**） |
| `~status` | 查看当前进度（含未处理上游反馈条数） |
| `~help` | 查看完整指令列表 |
| `~mode` | 切换介入模式 |
| `~settings` | 改目标平台/受众画像/视觉风格/**对标爆款**（v6 新增字段） |
| `~feedback <文本>` | v6 新增：给上游 agent 提修改意见，写入 `outputs/<集数>/feedback-to-upstream.md` |
