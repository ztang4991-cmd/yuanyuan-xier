...
## v12.1 — 2026-09-07 15:24:00 UTC+08:00

**主题**：接入 GPT-6 Astra 作为审核工作流第零步「规则层预审」，创作与规则审核跨模型家族交叉验证。

### ✨ 新增能力

1. **tools/astra-review.mjs**（Node，禁用 Python）：按阶段自动组装审核 skill / 方法论 / 技术约束 / 合规清单 / 项目配置 / 创作基准 / 上游参照 / 待审文件，strict JSON Schema 输出，每条问题 quote 原文回查，查不到即作废；verdict 由脚本按存活问题 + 评分阈值复算
2. **CLAUDE.md 审核工作流第零步**：Astra 预审 FAIL → 上游先修 → 再进 reviewer；中转不可用时跳过并标注
3. **reviewer.md 2.5 步**：读取 Astra 报告，重点转向审美/气质/方言/流量，可推翻并写明理由
4. **GPT-6-Astra接入定位建议报告.md**：只基于官方文档条目的能力对照与岗位论证（放验收、不放调度/编剧/音乐）
5. **.env.local + .gitignore**：密钥本地化，绝不入库

### 📊 实测（2026-09-07，番外01 导演稿）

- 4 条问题全部通过原文回查，含交接文档记录的已知问题（P02 三镜硬切违反 single-shot）
- 耗时 259s，19K in / 3K out；中转每请求注入约 7K 系统提示

### 📁 同批整理

- script/ 按产品线分组：10个争议脚本-30s / 四川话60s短剧 / 番外 / 99_讨论与诊断，全项目 29 处路径引用同步；新增 script/README.md 说明路径 B 下编号子目录为空的原因
